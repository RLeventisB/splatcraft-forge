...
public class SquidBumperModel extends EntityModel<SquidBumperEntity>
{
	...
	@Override
	public void animateModel(@NotNull SquidBumperEntity bumper, float limbSwing, float limbSwingAmount, float partialTick)
	{
		super.animateModel(bumper, limbSwing, limbSwingAmount, partialTick);
		
		scale = bumper.getBumperScale(MinecraftClient.getInstance().getRenderTickCounter().getTickDelta(true));
		Bumper.yaw = MathHelper.RADIANS_PER_DEGREE * MathHelper.lerp(partialTick, bumper.headYaw, bumper.prevHeadYaw) + (float) Math.PI;
