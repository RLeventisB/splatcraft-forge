...
public class SpawnShieldRenderer extends EntityRenderer<SpawnShieldEntity>
{
	...
	public void render(SpawnShieldEntity entity, float entityYaw, float partialTicks, @NotNull PoseStack matrixStack, @NotNull MultiBufferSource buffer, int packedLight)
	{
		...
		VertexConsumer builder = buffer.getBuffer(RenderType.entityTranslucentCull(getTextureLocation(entity)));// buffer.getBuffer(RenderLayer.entityTranslucentCull(getTextureLocation(entity)));//buffer.getBuffer(Minecraft.isFabulousGraphicsOrBetter() ? RenderLayer.translucentMovingBlock() : RenderLayer.translucentNoCrumbling());
	...
	@Override
	public @NotNull ResourceLocation getTextureLocation(@NotNull SpawnShieldEntity entity)
	{
