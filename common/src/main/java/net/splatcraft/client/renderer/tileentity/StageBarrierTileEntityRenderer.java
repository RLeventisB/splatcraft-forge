...
public class StageBarrierTileEntityRenderer implements BlockEntityRenderer<StageBarrierTileEntity>
{
	...
	/*public static final RenderType BARRIER_RENDER = RenderType.create("splatcraft:stage_barriers", DefaultVertexFormat.BLOCK, VertexFormat.Mode.QUADS, 262144, false, true, RenderType.CompositeState.builder()
		.setLightmapState(new RenderStateShard.LightmapStateShard(true)).setTextureState(new RenderStateShard.TextureStateShard(Splatcraft.identifierOf("textures/block/allowed_color_barrier_fancy.png"), false, true))
	...
	{
		builder.addVertex(matrixStack.last().pose(), x, y, z)
			.setColor(r, g, b, a)
	...
	public void render(StageBarrierTileEntity tileEntity, float partialTicks, @NotNull PoseStack matrixStack, @NotNull MultiBufferSource buffer, int combinedLight, int combinedOverlay)
	{
		...
		ResourceLocation registeredBlock = BuiltInRegistries.BLOCK.getKey(block);
		
		ResourceLocation textureLoc = Splatcraft.identifierOf("block/" + registeredBlock.getPath() + ((Minecraft.getInstance().options.graphicsMode().get().getId()) > 0 ? "_fancy" : ""));
		
		TextureAtlasSprite sprite = Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(textureLoc);
		VertexConsumer builder = buffer.getBuffer(Minecraft.useShaderTransparency() ? RenderType.translucentMovingBlock() : RenderType.translucent());
		...
		{
			InkColor color = ColorUtils.getColorLockedIfConfig(ColorUtils.getEffectiveColor(tileEntity.getLevel(), tileEntity.getBlockPos()));
			rgb = color.getRGB();
		}
