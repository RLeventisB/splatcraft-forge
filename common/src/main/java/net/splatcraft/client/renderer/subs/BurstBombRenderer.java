...
public class BurstBombRenderer extends SubWeaponRenderer<BurstBombEntity, BurstBombModel>
{
	...
	public void render(BurstBombEntity entityIn, float entityYaw, float partialTicks, @NotNull PoseStack matrices, @NotNull MultiBufferSource bufferIn, int packedLightIn)
	{
		...
		if (!entityIn.isItem)
		{
			...
			matrices.mulPose(Axis.YP.rotationDegrees(Mth.lerp(partialTicks, entityIn.yRotO, entityIn.getYRot()) - 180.0F));
			matrices.mulPose(Axis.XP.rotationDegrees(Mth.lerp(partialTicks, entityIn.xRotO, entityIn.getXRot()) + 90F));
