...
public class InkStainedBlock extends Block implements IColoredBlock, BlockEntityProvider
{
	...
	public @NotNull ItemStack getPickStack(@NotNull WorldView world, @NotNull BlockPos pos, @NotNull BlockState state)
	{
		...
		if (color.isValid())
			return ColorUtils.withInkColor(super.getPickStack(world, pos, state), color);
		return ColorUtils.withColorLocked(ColorUtils.withInkColor(super.getPickStack(world, pos, state), color), true);
	}
	@Override
