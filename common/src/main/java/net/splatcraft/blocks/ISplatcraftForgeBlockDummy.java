...
public interface ISplatcraftForgeBlockDummy
{
    ...
    default boolean shouldCheckWeakPower(RedstoneView level, BlockPos pos, Direction side) {return true;}

    default float[] getBeaconColorMultiplier(BlockState state, WorldAccess level, BlockPos pos, BlockPos beaconPos) {return new float[0];}

    default Optional<Vec3d> getRespawnPosition(BlockState state, EntityType<?> type, WorldView world, BlockPos pos, float orientation, @Nullable LivingEntity entity) {return Optional.empty();}

    default float getExplosionResistance(BlockState state, BlockView level, BlockPos pos, Explosion explosion) {return 0f;}

    default boolean addLandingEffects(BlockState state1, ServerWorld levelserver, BlockPos pos, BlockState state2, LivingEntity entity, int numberOfParticles) {return true;}

    default boolean addRunningEffects(BlockState state, World world, BlockPos pos, Entity entity) {return true;}

    default boolean canHarvestBlock(BlockState state, BlockView level, BlockPos pos, PlayerEntity player) {return true;}

    default boolean collisionExtendsVertically(BlockState state, BlockView level, BlockPos pos, Entity collidingEntity) {return false;}
