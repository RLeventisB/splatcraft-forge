...
public class ReplaceColorCommand
{
	...
	public static int execute(CommandContext<CommandSourceStack> context, int mode) throws CommandSyntaxException
	{
		if (mode == 0)
		{
			return execute(context.getSource(), BlockPosArgument.getLoadedBlockPos(context, "from"), BlockPosArgument.getLoadedBlockPos(context, "to"), InkColorArgument.getInkColor(context, "color"), InkColor.INVALID, mode, "", "");
		}
	...
	public static int execute(CommandSourceStack source, BlockPos from, BlockPos to, InkColor color, InkColor affectedColor, int mode, String affectedStage, String affectedTeam)
	{
