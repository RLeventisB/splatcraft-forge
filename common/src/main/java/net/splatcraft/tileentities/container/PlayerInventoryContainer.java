...
public abstract class PlayerInventoryContainer<T extends PlayerInventoryContainer<?>> extends AbstractContainerMenu
{
	...
	{
		return stillValid(levelPosCallable, playerIn, SplatcraftBlocks.weaponWorkbench.get());
	}
	@Override
	public @NotNull ItemStack quickMoveStack(@NotNull Player playerIn, int index)
	{
		...
		{
			if (!moveItemStackTo(stack, slots.size() - 9, slots.size(), true))
			{
				return ItemStack.EMPTY;
			}
		}
		...
		{
			if (!moveItemStackTo(stack, 0, slots.size() - 9, false))
			{
				return ItemStack.EMPTY;
			}
		}
