...
public class SardiniumDepositFeature extends Feature<NoneFeatureConfiguration>
{
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context)
	{
		...
		centerPos = new BlockPos(centerPos.getX(), worldAccess.getHeight(Heightmap.Types.OCEAN_FLOOR, centerPos.getX(), centerPos.getZ()), centerPos.getZ());
	...
	private void generateIcebergBlock(LevelAccessor level, RandomSource raqndom, BlockPos p_66061_, int p_66062_, int p_66063_, int p_66064_, int p_66065_, int p_66066_, int p_66067_, boolean p_66068_, int p_66069_, double p_66070_, BlockState state)
	{
	...
	private void smooth(LevelAccessor access, BlockPos pos, int p_66054_, int p_66055_, boolean p_66056_, int p_66057_)
	{
			...
			for (int k = -i; k <= i; ++k)
			{
				for (int l = 0; l <= p_66055_; ++l)
				{
					...
					if (isIcebergState(blockstate) || blockstate.is(Blocks.SNOW))
					{
						...
						{
							setBlock(access, blockpos, Blocks.AIR.defaultBlockState());
							setBlock(access, blockpos.above(), Blocks.AIR.defaultBlockState());
						}
	...
	private boolean belowIsAir(BlockGetter access, BlockPos pos)
	{
		return access.getBlockState(pos.below()).isAir();
	}
}
