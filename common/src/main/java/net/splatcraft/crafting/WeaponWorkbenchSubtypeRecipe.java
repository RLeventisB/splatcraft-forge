...
public class WeaponWorkbenchSubtypeRecipe extends AbstractWeaponWorkbenchRecipe
{
	public static final Codec<WeaponWorkbenchSubtypeRecipe> CODEC = RecordCodecBuilder.create(inst ->
		inst.group(
			...
			Identifier.CODEC.optionalFieldOf("advancement").forGetter(v -> v.advancement),
	...
	public static final PacketCodec<RegistryByteBuf, WeaponWorkbenchSubtypeRecipe> PACKET_CODEC = PacketCodec.ofStatic(
		(buffer, recipe) ->
		{
			...
			PacketCodecs.optional(Identifier.PACKET_CODEC).encode(buffer, recipe.advancement);
		...
		(buffer) ->
			new WeaponWorkbenchSubtypeRecipe(
				...
				PacketCodecs.optional(Identifier.PACKET_CODEC).decode(buffer),
	...
	private final Optional<Identifier> advancement;
	private final boolean requireOther;
	public WeaponWorkbenchSubtypeRecipe(Text name, ItemStack recipeOutput, List<StackedIngredient> recipeItems, Optional<Identifier> advancement, boolean requireOther)
	{
	...
	public boolean isAvailable(PlayerEntity player)
	{
		...
		{
			AdvancementEntry advancementEntry = serverPlayer.getServer().getAdvancementLoader().get(advancement.get());
	...
	{
		if (!(player instanceof ClientPlayerEntity clientPlayer) || advancement.isEmpty())
			return true;
		
		AdvancementEntry advancement = clientPlayer.networkHandler.getAdvancementHandler().get(this.advancement.get());
