...
public class RollerRollTickableSound extends AbstractTickableSoundInstance
{
	...
	public void tick()
	{
		if (this.player.isAlive() && player.getUseItem().getItem() instanceof RollerItem)
		{
			...
			if (!WeaponBaseItem.enoughInk(player, roller.getItem(), Math.max(((RollerItem) roller.getItem()).getSettings(roller).rollConsumption, ((RollerItem) roller.getItem()).getSettings(roller).dashConsumption), 7, false))
			{
				stop();
				return;
			}
