...
public class BurstBombRenderer extends SubWeaponRenderer<BurstBombEntity, BurstBombModel>
{
	...
	public void render(BurstBombEntity entityIn, float entityYaw, float partialTicks, @NotNull PoseStack PoseStackIn, @NotNull MultiBufferSource bufferIn, int packedLightIn)
	{
		...
		if (!entityIn.isItem)
		{
			...
			PoseStackIn.mulPose(VectorUtils.rotationDegrees(VectorUtils.YP, Mth.lerp(partialTicks, entityIn.yRotO, entityIn.getYRot()) - 180.0F));
			PoseStackIn.mulPose(VectorUtils.rotationDegrees(VectorUtils.XP, Mth.lerp(partialTicks, entityIn.xRotO, entityIn.getXRot()) + 90F));
