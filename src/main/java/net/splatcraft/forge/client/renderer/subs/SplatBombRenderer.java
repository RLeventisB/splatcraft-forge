...
public class SplatBombRenderer extends SubWeaponRenderer<SplatBombEntity, SplatBombModel>
{
	...
	public void render(SplatBombEntity entityIn, float entityYaw, float partialTicks, @NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferIn, int packedLightIn)
	{
		...
		if (!entityIn.isItem)
		{
			...
			poseStack.mulPose(VectorUtils.rotationDegrees(VectorUtils.YP, Mth.lerp(partialTicks, entityIn.yRotO, entityIn.getYRot()) * 2 - 90f));
			poseStack.mulPose(VectorUtils.rotationDegrees(VectorUtils.XP, Mth.lerp(partialTicks, entityIn.xRotO, entityIn.getXRot()) * 2 - 180f));
