...
public class SplatcraftOreGen
{
	...
	/*public static void registerOres(BootstapContext<ConfiguredFeature<?, ?>> context)
	{
		Holder<ConfiguredFeature<CountConfiguration, ?>> crate_small = FeatureUtils.register(context, ResourceKey.createRegistryKey(new ResourceLocation(Splatcraft.MODID, "crate_small")), crate_feature.get(), new CountConfiguration(8));
		Holder<ConfiguredFeature<CountConfiguration, ?>> crate_large = FeatureUtils.register(context, Splatcraft.MODID + ":crate_large", crate_feature.get(), new CountConfiguration(12));
		
		Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> sardinium_deposit = FeatureUtils.register(context, Splatcraft.MODID + ":sardinium_deposit", sardinium_deposit_feature.get(), new NoneFeatureConfiguration());
		
		oceanGen.add(PlacementUtils.register(context, Splatcraft.MODID + ":crate_small", crate_small, RarityFilter.onAverageOnceEvery(16), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP_TOP_SOLID, BiomeFilter.biome()));
		oceanGen.add(PlacementUtils.register(context, Splatcraft.MODID + ":crate_large", crate_large, RarityFilter.onAverageOnceEvery(28), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP_TOP_SOLID, BiomeFilter.biome()));
		oceanGen.add(PlacementUtils.register(context, Splatcraft.MODID + ":sardinium_deposit", sardinium_deposit, RarityFilter.onAverageOnceEvery(32), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP_TOP_SOLID, BiomeFilter.biome()));
	}
	@SubscribeEvent
